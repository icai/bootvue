...
require('webpack-dev-server/lib/polyfills');
...
const createDomain = require('webpack-dev-server/lib/util/createDomain'); // eslint-disable-line
...
function startDevServer(webpackOptions, options) {
  ...
  // var watcher = watch('./src/pages/**/app.@(js)', {root: path.resolve(__dirname, '../')})
  // watcher.on('add', function(spath, stat) {
  //   let cpath = spath.replace(/\\/g, '/')
  //   const chunk = cpath.split('src/pages/')[1].split(/\/app\.js/)[0];
  //   // console.info(chunk, cpath)
  //   // require("./dynamic-html").dynamicHtmlPluginAdd(cpath, webpackOptions);
  //   compiler.apply(new SingleEntryPlugin(webpackOptions.context, cpath, chunk));

  //   compiler.apply.apply(compiler, webpackOptions.plugins);
  //   server.invalidate();
  // });
